...
Funcion main
	...
	sprite = SpriteString_AddLine(sprite, ":Џ;     _  ____   _. ._ __  __"); 
	sprite = SpriteString_AddLine(sprite, "| |    | ||  _ \ | | | |\ \/ /");
	sprite = SpriteString_AddLine(sprite, "| |___ | || | | || |_| | >  < ");
	sprite = SpriteString_AddLine(sprite, "|_____||_||_| |_| \___/ /_/\_\");
	...
	canvas = canvas_DrawText(canvas, Cx, Cy, "Sistemas Operativos Presentacion", 1, 7);
	canvas = canvas_DrawHorizontalLine_Full(canvas, Cx, Cy, 8);
	canvas = canvas_DrawButton(canvas, Cx, Cy, " 1. Linux Mint ", 7, 9);
	canvas = canvas_DrawPanel(canvas, Cx, Cy, 0, 0, Cx, Cy);
	canvas_Display(canvas, Cx, Cy);
//	tui = TUI_New(Cx, Cy);
//	tui = TUI_AddTComponent_NewPanel(tui, "panel_Border", 0,0, Cx, Cy);
//	tui = TUI_AddTComponent_NewSpriteString(tui, "Sprite", sprite, Sprite_EFFECT_NONE(), 5, 1);
//	tui = TUI_AddTComponent_NewLabel(tui, "label_Menu", "Aeropuerto UNI", 8, 7);
//	tui = TUI_AddTComponent_NewHorizontalRuler(tui, "hr_menu", 0, 6, Cx);
//	tui = TUI_AddTComponent_NewHorizontalRuler(tui, "hr_men2", 0, 8, Cx);
//	tui = TUI_AddTComponent_NewButton(tui, "boton_ayuda", " 1. ingresar ", "1", 7, 10);
//	TUI_Display(tui);
FinFuncion
...
//1--PRINTERS  З  З  З  З  З  З #4 		        	print_ : prinln_ 		[#]
...
//1--LOCAL_DATE_TIME З  З  З  З	#+1					Local_					[D]
//1    \--DURATION   З  З  З  З ++1_1				Duration_				[P]
...
//1--canvas    З  З  З  З  З  З	#+4  			 	canvas_					[+]
//1--TUI	   З  З  З  З  З  З #+5   				TUI_					[+]
...
Funcion result_string <-  string_pad_start(text, text_pad)
	Definir result_string Como Texto;
	result_string=string_insert(text, string_substring_from_start(text_pad, math_max_Int(string_Length(text_pad)-string_Length(text), 0)), 0);
FinFuncion
...
Funcion text_Revert <- string_TextRevert(text)
	...
	Para i=0 Hasta indexText Con Paso 1 Hacer
		text_Revert = string_append(text_Revert, char_At(text, indexText-i));
	FinPara
FinFuncion
...
Funcion text_Revert <- string_TextRevert_Caracters(text, array_Carathers, array_size)
    ...
    Mientras i <= indexText  Hacer
        ...
        si !string_isEquals(Char_Current, pixel_clear()) Entonces 
			Char_Current = __private_string_CharRevert_Caracters(Char_Current, array_Carathers, array_size);
		FinSi
...
Funcion char_Revert <- __private_string_CharRevert_Caracters(char_, array_Carathers, array_size)
	Definir char_Revert Como Texto;
	Definir i Como Numero;
	char_Revert = char_;
	si !string_isEquals(char_Revert, " ")  Entonces
		Mientras i < array_size Hacer
			Si char_Revert == array_Carathers[i] Entonces
				Si math_module(i, 2) == 0 Entonces
					char_Revert = array_Carathers[increment(i)];
				SiNo
					char_Revert = array_Carathers[decrement(i)];
				FinSi
				i = array_size;
			SiNo
				i = increment(i);
...
Funcion strSubs <- string_substring(result_string, start, end)
	Definir strSubs Como Texto;
	validate_index_range("string_substring", result_string, start, end);
...
Funcion test_Assert_Equal(value, value_expected, TYPE)
	...
	test_funtion_Assert_Equal(value, value_expected, TYPE, "<?>");
FinFuncion

Funcion test_funtion_Assert_Equal(value, value_expected, TYPE, method_name)
	Definir catch_Boolean Como Logico;
	catch_Boolean = test_funtion_Assert_Equal_returnBoolean(value, value_expected, TYPE, method_name);
FinFuncion

Funcion result_Boolean <- test_funtion_Assert_Equal_returnBoolean(value, value_expected, TYPE, method_name)
	Definir value_String, value_expected_String Como Texto;
...
Funcion count_result <- test_Passed(value, value_expected, TYPE, count_string)
	...
	count_result = test_funtion_Passed(value, value_expected, TYPE, "<?>", count_string);
FinFuncion

Funcion count_result <- test_funtion_Passed(value, value_expected, TYPE, method_name, count_string)
	Definir count_result Como Texto;
	Definir catch_Boolean Como Logico;
	catch_Boolean = test_funtion_Assert_Equal_returnBoolean(value, value_expected, TYPE, method_name);
...
Funcion test_funtion_Passed_Debug(count_string)
	...
	succes_Msg = string_append("І  + SUCCESS: ", string_fit_end(succes_Count, "            І"));
	error_Msg = string_append("І  - ERROR:   ", string_fit_end(error_Count, "            І"));
	Dimension array_test[6];
	array_test[0] = ".=========================.";
	array_test[1] = "І      < END_TEST >        І";
	array_test[2] = ":=========================:";
...
Funcion validate_index_range(funtion_name, result_string, start, end)
	Definir num_lentgh Como Numero;
	Definir Dates Como Texto;
	DimenSion Dates[3];
	num_lentgh = string_Length(result_string);
...
Funcion tcomponent <- TProperty_Enable
	Definir tcomponent Como texto;
	tcomponent = "enable";
FinFuncion
//... . . . type_Tcomponent
...
Funcion element_Result <- linearCollection_getFromSide(struct_Collection, Side, TYPE)
	Definir element_String Como Texto;
	Si Side < 1 Entonces
		element_String = linearCollection_getFirts_ToString(struct_Collection);
	SiNo
...
Funcion element_Result <- linearCollection_getElement(struct_Collection, index_element, TYPE)
	Definir element_String Como Texto;
...
Funcion info_index_element <- linearCollection_Build_IndexInfoString(struct_List, index_element)
	...
	Meta_Data = string_substring(struct_List, increment(collection_getIndex_MetaDataArea(struct_List)), linearCollection_getIndex_EndMetaData(struct_List));
	num_carry = "";
	char_count = 0;
	i = 0;
	...
	Mientras char_count >= 0 & (index_Element >= i)  Hacer
		char_Current = char_At(Meta_Data, char_count);
...
Funcion element_String <- linearCollection_getElement_toString(struct_Collection, index_element)
...
Funcion collection_result <- linearCollection_RemoveElement(struct_Collection, index_element)
	Definir collection_result Como Texto;
	collection_result = linearCollection_ModifyElement(struct_Collection, index_element, string_Null());
	collection_result = linearCollection_decrement_numElement(collection_result);
FinFuncion
...
Funcion collection_result <- linearCollection_SetElement_ToString(struct_Collection, index_element, value_string)
	...
	collection_result = linearCollection_ModifyElement(struct_Collection, index_element, value_string);
FinFuncion

Funcion collection_result <- linearCollection_ModifyElement(struct_Collection, index_element, modify_value)
	...
	Si linearCollection_Index_IsValid(struct_Collection, index_element) Entonces
		...
		FinSi
	SiNo
		error_Message_Funtion("linearCollection_ModifyElement", string_append("index_element no is valid Size:", num_ToString(linearCollection_GetSize(struct_Collection))));
		collection_result = struct_Collection;
	FinSi
FinFuncion

Funcion innerIndex <- LinearCollection_GetValue_InnerIndex(linearCollection, data_string)
	Definir separated_String, string_Sumation, dataSoup Como Texto; 
	...
	dataSoup = string_substring(linearCollection, index_Data, index_MetaData);
	...
	innerIndex = __private_CollectionSetter_GetID_DataString(dataSoup, string_Sumation, collection_size, data_string);
...
Funcion separated_String <- LinearCollection_GetValues_ToSeparatedString(linearCollection, separator)
	...
	dataSoup = string_substring(linearCollection, index_Data, index_MetaData);
	string_Sumation= string_substring(linearCollection, increment(index_MetaData), index_ExtraData);
	separated_String = __private_LinearCollection_GetDataSeparatedString(dataSoup, string_Sumation, separator);
FinFuncion

Funcion separated_String <- __private_LinearCollection_GetDataSeparatedString(dataSoup, string_Sumation, separator)
	Definir separated_String, num_carry, Char_Current, data_Current Como Texto;
	Definir i, num_range, num_carry_int Como Numero;
	Definir length_Sumation Como Numero;
	Definir IsNumber Como Logico;
	num_carry 	     = "";
	separated_String = "";
	num_range		 = 0;	
	...
	Mientras i <= length_Sumation Hacer
		...
		Si !IsNumber & !string_isEmpty(num_carry) | Num_isEquals(i, length_Sumation) Entonces
			num_carry_int   = string_ToNum(num_carry);
			data_Current     = string_substring(dataSoup, num_range, increment_step(num_range, num_carry_int));
...
Funcion deque_Result <- util_Deque_AddFirst(struct_Deque, element)
	...
	deque_Result = linearCollection_addFirts(struct_Deque, element);
FinFuncion
...
Funcion collection_result <- util_Deque_RemoveFirts(struct_Deque)
	Definir collection_result Como Texto;
	collection_result = linearCollection_RemoveFirts(struct_Deque);
FinFuncion
...
Funcion collection_result <- util_Queue_RemoveFirst(struct_Queue)
	Definir collection_result Como Texto;
	collection_result = linearCollection_RemoveFirts(struct_Queue);
FinFuncion
...
Funcion key_id <- __private_CollectionSetter_SearchIdKey(collection_setter, key_string, collection_size)
	...
	Definir keys_collection, dataSoup, string_Sumation Como Texto;
	si collection_size >= 1 Entonces
		...
		dataSoup            = string_substring(collection_setter, increment(index_KeyArea), index_EndKeyArea);
		key_id = __private_CollectionSetter_GetID_DataString(dataSoup, string_Sumation, collection_size, string_ToLowerCase(key_string));
	SiNo
		key_id = number_Null();
	FinSi
FinFuncion

Funcion inner_IdData <- __private_CollectionSetter_GetID_DataString(dataSoup, string_Sumation, max_NumComparations, data_string)
	Definir separated_String, num_carry, Char_Current, data_Current Como Texto;
	...
	Mientras i <= length_Sumation & !BreakLoop_Data & max_NumComparations >= count_Comparations Hacer
		...
		Si !IsNumber & !string_isEmpty(num_carry) | Num_isEquals(i, length_Sumation) Entonces
			...
			si Num_isEquals(length_innerData, length_data)  Entonces
				data_Current     = string_substring(dataSoup, num_range, increment_step(num_range, length_innerData));
...
Funcion keys_collection <- CollectionSetter_GetKeys_ToSeparatedString(collection_setter, separator)
    ...
    Si count_size > 0 Entonces
        keys_collection = __private_LinearCollection_GetDataSeparatedString(dataSoup, string_Sumation, separator);
    FinSi
FinFuncion
...
Funcion canvas_rectangle <- canvas_DrawRectangle_normaliced(canvas, CWx, CWy, x0, y0, x1, y1)
...
Funcion canvas_rectangle <- canvas_DrawRectangle(canvas, Cx, Cy, x0, y0, width, heigth)
	Definir canvas_rectangle Como Texto;
	canvas_rectangle = canvas_DrawRectangle_Pixel(canvas, Cx, Cy, pixel_plain(), x0, y0, width, heigth);
FinFuncion

Funcion canvas_rectangle <- canvas_DrawRectangle_Pixel(canvas, Cx, Cy, pixelVH, x0, y0, width, heigth)
	Definir canvas_rectangle Como Texto;
	canvas_rectangle = canvas_DrawRectangle_Pixels(canvas, Cx, Cy, pixelVH, pixelVH, x0, y0, width, heigth);
FinFuncion

Funcion canvas_rectangle <- canvas_DrawRectangle_Pixels(canvas, Cx, Cy, pixel_H, pixel_V, x0, y0, width, heigth)
    Definir canvas_rectangle Como Texto;
	canvas_rectangle = canvas_DrawRectangle_Pixels_Full(canvas, Cx, Cy, pixel_H, pixel_H, pixel_V, pixel_V, x0, y0, width, heigth);
FinFuncion
...
Funcion canvas_triangle <- canvas_DrawTriangle_normaliced(canvas, CWx, CWy, x0, y0, x1, y1, x2, y2)
...
Funcion canvas_point <- canvas_DrawPoint_normaliced(canvas, CWx, CWy, x0, y0)
	Definir canvas_point Como Texto;
	canvas_point = canvas_DrawPoint_Pixel_normaliced(canvas, CWx, CWy, x0, y0, pixel_plain());
FinFuncion

Funcion canvas_point <- canvas_DrawPoint_Pixel_normaliced(canvas, CWx, CWy, x0, y0, pixel_symb)
	Definir canvas_point Como Texto;
	x0 = canvas_Noramalized_ToPixelX(CWx, x0);
	y0 = canvas_Noramalized_ToPixelY(CWy, y0);
...
Funcion canvas_text <- canvas_DrawText(canvas, CWx, CWy, text, x0, y0)
	...
	si x0 < CWx & y0 < CWy & y0 >= 0 Entonces 
		text = __private_canvas_text_Normalize(text, x0, CWx);
		post_text = canvas_getIndex(x0, y0, CWx);
		canvas_text = string_insert_withReplace(canvas_text, text, post_text);
	FinSi
FinFuncion

Funcion text_norm <- __private_canvas_text_Normalize(text, x0, CWx)
...
Funcion canvas_line <- canvas_DrawLine_normaliced(canvas, CWx, CWy, x0, y0, x1, y1)
...
Funcion canvas_Result <- canvas_DrawButton(canvas, Cx, Cy, text, x0, y0)
	...
	y1 = increment(yt);
	canvas_Result = canvas_DrawRectangle_Pixels_Full(canvas_Result, Cx, Cy, "_","Џ", "|", "|", x0, y0, x1, y1);
...
Funcion canvas_Result <- canvas_DrawPanel(canvas, Cx, Cy, x0, y0, width, heigth)
	Definir canvas_Result Como Texto;
	canvas_Result = canvas;
	canvas_Result  = canvas_DrawVerticalLine(canvas_Result, Cx, Cy, x0, y0, heigth);
	canvas_Result = canvas_DrawVerticalLine(canvas_Result, Cx, Cy, decrement(width), y0, heigth);
	canvas_Result = canvas_DrawHorizontalLine(canvas_Result, Cx, Cy, x0, y0, width);
	heigth = decrement(heigth);
	canvas_Result = canvas_DrawHorizontalLine(canvas_Result, Cx, Cy, x0, heigth, width);
	canvas_Result = canvas_DrawPoint_pixel(canvas_Result, Cx, Cy, "з", x0, y0);
	canvas_Result = canvas_DrawPoint_pixel(canvas_Result, Cx, Cy, "з", x0, heigth);
	width = decrement(width);
	canvas_Result = canvas_DrawPoint_pixel(canvas_Result, Cx, Cy, "з", width, heigth);
	canvas_Result = canvas_DrawPoint_pixel(canvas_Result, Cx, Cy, "з", width, y0);
FinFuncion
...
Funcion canvas_Result <- canvas_DrawVerticalLine(canvas, Cx, Cy, x0, y0, height)
	Definir canvas_Result Como Texto;
	canvas_Result = canvas_DrawVerticalLine_Pixel(canvas, Cx, Cy, "І", x0, y0, height);
FinFuncion
...
Funcion canvas_Sprite <- __private_canvas_DrawSpriteString_sprite_effect(canvas, Cx, Cy, SpriteString_Structure, x0, y0, symbol_Trimmed, sprite_effect)
	Definir canvas_Sprite, string_Sumation, dataSoup Como Texto; 
	Definir index_Data, index_MetaData, index_ExtraData, size_Sprite Como Numero;
	size_Sprite 	 = SpriteString_GetHeigth(SpriteString_Structure);
	SpriteString_Structure = linearCollection_RemoveFirts(SpriteString_Structure);
	...
	dataSoup		 = string_substring(SpriteString_Structure, index_Data, index_MetaData);
	string_Sumation  = string_substring(SpriteString_Structure, increment(index_MetaData), index_ExtraData);
	canvas_Sprite 	 = __private_Canvas_RenderSprite(canvas, Cx, Cy, dataSoup, string_Sumation,  size_Sprite, symbol_Trimmed, x0, y0, sprite_effect);
FinFuncion

Funcion canvas_Sprite <- __private_Canvas_RenderSprite(canvas, Cx, Cy, dataSoup, string_Sumation, size_Sprite, symbol_Trimmed, x0, y0, sprite_effect)
	Definir canvas_Sprite, num_carry, Char_Current, line_Current, array_sprite Como Texto;
	Definir i, num_range, num_range_Inc, num_carry_int, length_Sumation, count_line Como Numero;
	Definir IsNumber Como Logico;
	...
	Mientras i <= length_Sumation & size_Sprite < Cy Hacer
		...
		Si !IsNumber & !string_isEmpty(num_carry) | Num_isEquals(i, length_Sumation) Entonces
			num_carry_int = string_ToNum(num_carry);
			num_range_Inc = increment_step(num_range, num_carry_int);
			line_Current  = string_substring(dataSoup, num_range, num_range_Inc);
...
Funcion canvas_Sprite <- Canvas_ApplyRenderEffect(canvas, Cx, Cy, x0, y0, array_sprite, size_Sprite, symbol_Trimmed, sprite_effect)
    ...
    Segun sprite_effect Hacer
        Sprite_EFFECT_NONE():
            canvas_Sprite = canvas_DrawSpriteString_Array(canvas, Cx, Cy, x0, y0, array_sprite, size_Sprite);
...
Funcion canvas_Sprite <- canvas_DrawSpriteString_Array(canvas, Cx, Cy, x0, y0, array_sprite, size)	
	Definir canvas_Sprite Como Texto;
	Definir i Como Numero;
	canvas_Sprite = canvas;
	size = size;
...
Funcion canvas_Sprite <- canvas_DrawSprite_Trimmed(canvas, Cx, Cy, x0, y0, array_sprite, size, symbol_Trimmed)	
	...
	Mientras i < size & increment_step(i, y0) < Cy Hacer
		...
		i = increment(i);
	FinMientras
	canvas_Sprite = canvas_DrawSpriteString_Array(canvas, Cx, Cy, x0, y0, array_sprite, size);
FinFuncion
...
Funcion canvas_Sprite <- canvas_DrawSprite_Cutout_FlipHorizontal(canvas, Cx, Cy, x0, y0, array_sprite, size)	
	...
	i = 0;
	Mientras i < size & increment_step(i, y0) < Cy Hacer
		array_sprite[i] = string_TextRevert_Caracters(array_sprite[i], array_symbol, size_symb);
		i = increment(i);
	FinMientras
...
Funcion canvas_Sprite <- canvas_DrawSprite_Cutout_FlipVertical(canvas, Cx, Cy, x0, y0, array_sprite, size)	
    ...
    array_symbol[4] = "[";array_symbol[5] = "]";
    array_symbol[6] = "{";array_symbol[7] = "}";
    array_symbol[8] = "_";array_symbol[9] = "Џ";
	Dimension array_column_invert[size];
	Mientras i < size & increment_step(i, y0) < Cy Hacer
		array_column_invert[i] = string_TextRevert_Caracters(array_sprite[decrement_step(size, increment(i))], array_symbol, size_symb);
		i = increment(i);
	FinMientras
...
Funcion tui_TFrame <- TUI_New(Wx,Wy)
	...
	stores_Tcomponents = collectionSetter_addValueFuntion(collectionSetter_New(TUI_TYPE_TSTORE(), TYPE_STRING()), TYPE_STRING());
	size_TFrame = TUI_ParserPointXY_ToString(Wx, Wy);
	tui_TFrame  = linearCollection_addFirts(tui_TFrame, stores_Tcomponents);//index:2
	tui_TFrame  = linearCollection_addFirts(tui_TFrame, collectionSetter_New(TUI_TYPE_TINPUT(),TYPE_STRING()));//index:1
	tui_TFrame  = linearCollection_addFirts(tui_TFrame, size_TFrame);//index:0
FinFuncion
...
Funcion TUI_Display(tui_window)
	...
	Si TUI_IsInterface_Valid(tui_window) Entonces 
		Wsize = TUI_GetTProperty_Size(tui_window);
...
// -- -- -- -- -- [ Get ] -- -- -- -- -- --
Funcion size_String <- TUI_GetTProperty_Size(tui_TFrame) 
	Definir size_String Como texto;
	size_String = linearCollection_getFirts_ToString(tui_TFrame);
FinFuncion
...
Funcion user_input <- TUI_GetUser_Input_Message(tui_window, message, TYPE)
	...
	SiNo
		error_Message_Funtion("TUI_GetUser_Input_message(tui_window, message, TYPE)","Its not a TUI interface");
...
Funcion tui_resize <- TUI_SetSize(tui_window, Wx, Wy) 
	...
	size_string = TUI_ParserPointXY_ToString(Wx, Wy);
...
Funcion TUI_result <- TUI_Tcomponent_SetTproperty_Position(tui_window, reference_Name, x0, y0) 
	...
	point_String = TUI_ParserPointXY_ToString(x0, y0);
...
Funcion TUI_result <- TUI_Tcomponent_SetTproperty_Position_TwoPoints(tui_window, reference_Name, x0, y0, x1, y1) 
	...
	points_String = TUI_ParserTwoPointsXY_ToString(x0, y0, x1, y1);
...
FinFuncion
// -- -- -- -- -- [ POINT_XY ] -- -- -- -- -- --
Funcion tcomponent_Point <- TUI_ParserPointXY_ToString(x0, y0)
...
Funcion tcomponent_Point_Full <- TUI_ParserTwoPointsXY_ToString(x0, y0, x1, y1)
	Definir tcomponent_Point, tcomponent_Point2, tcomponent_Point_Full Como texto;
	tcomponent_Point = TUI_ParserPointXY_ToString(x0, y0);
	tcomponent_Point2 = TUI_ParserPointXY_ToString(x1, y1);
...
Funcion TUI_result <- TUI_AddTcomponent(tui_window, reference_Name, tcomponent_element)
	...
	SiNo
		...
		error_Message_Funtion("TUI_AddTcomponent","Its not a TUI interface");
	FinSi
FinFuncion
...
Funcion TUI_result <- TUI_AddTcomponent_TypeInput_Forced(tui_window, reference_Name, tcomponent_element)
	...
	si collectionSetter_Exist_ByString(input_TUI, input_Tcomponent) Entonces
		TUI_result = tui_window;
		error_Message_Funtion("TUI_AddTcomponent_TypeInput_Forced(tui, name, tcomponent)","exist input");
	SiNo
		TUI_result = TUI_AddTcomponent_Forced(tui_window, reference_Name, tcomponent_element);
...
Funcion tui_NewTcomponent <- TUI_AddTComponent_NewButton(tui_window, reference_Name, text, input_string, x0, y0)
	...
	SiNo
		error_Message_Funtion("TUI_AddTComponent_NewButton()", "Its not a TUI interface");
	FinSi
FinFuncion
...
Funcion tui_NewTcomponent <- TUI_AddTComponent_NewLabel(tui_window, reference_Name, text, x0, y0)
	...
	SiNo
		error_Message_Funtion("TUI_AddTComponent_NewLabel()", "Its not a TUI interface");
	FinSi
FinFuncion
...
Funcion tui_NewTcomponent <- TUI_AddTComponent_NewHorizontalRuler(tui_window, reference_Name, x0, y0, value)
	...
	SiNo
		error_Message_Funtion("TUI_AddTComponent_NewHorizontalRuler()", "Its not a TUI interface");
	FinSi
FinFuncion
...
Funcion tui_NewTcomponent <- TUI_AddTComponent_NewVerticalRuler(tui_window, reference_Name, x0, y0, value)
	...
	SiNo
		error_Message_Funtion("TUI_AddTComponent_NewVerticalRuler()", "Its not a TUI interface");
	FinSi
FinFuncion
...
Funcion tui_NewTcomponent <- TUI_AddTComponent_NewPanel(tui_window, reference_Name, x0, y0, x1, y1)
	...
	SiNo
		error_Message_Funtion("TUI_AddTComponent_NewPanel()", "Its not a TUI interface");
	FinSi
FinFuncion
...
Funcion TUI_result <- TUI_TcomponentInput_Remove_Forced(tui_window, reference_Name) 
	Definir TUI_result, Store_Setter, input_TUI, tcomponent_element, input_Tcomponent Como texto;
	Store_Setter = TUI_GetTProperty_Store(tui_window);
	input_TUI	 = TUI_GetTProperty_Input(tui_window);
...
Funcion TUI_result <- TUI_Tcomponent_Remove_Forced(tui_window, reference_Name) 
	Definir TUI_result, Store_Setter Como texto;
	Store_Setter = TUI_GetTProperty_Store(tui_window);
	Store_Setter = collectionSetter_RemoveKeyValue_ByString(Store_Setter, reference_Name);
	TUI_result   = linearCollection_SetElement_ToString(tui_window, TindexFrame_Store(), Store_Setter);
...
Funcion isTUI <- TUI_IsInterface_Valid(interface_string)
	...
	isTUI = string_isEquals(collection_getName_TypeCollection(interface_string), TUI_TYPE_TFRAME());
FinFuncion
// -- -- -- -- -- [ TUI_TO_canvas ] -- -- -- -- -- --
Funcion new_canvas <- TUI_ToCanvas(tui_window)
	...
	SiNo
		error_Message_Funtion("TUI_Tocanvas(tui_window)","Its not a TUI interface");
	FinSi
FinFuncion

Funcion new_canvas <- __private_canvas_DrawLinearCollection_Tcomponent(canvas, Cx, Cy, TlinearCollection)
	Definir new_canvas, dataSoup, string_Sumation Como Texto;
	...
	dataSoup = string_substring(TlinearCollection, index_DataArea, index_MetaData);
	string_Sumation= string_substring(TlinearCollection, increment(index_MetaData), index_ExtraData);
	new_canvas = __private_Tocanvas_ByDataSoup(canvas, Cx, Cy, dataSoup, string_Sumation);
FinFuncion

Funcion new_canvas <- __private_Tocanvas_ByDataSoup(canvas, Cx, Cy, dataSoup, string_Sumation)
	Definir new_canvas, num_carry, Char_Current, data_Current Como Texto;
	Definir i, index_Tcomponent, index_Tcomponent_Dec, num_carry_int Como Numero;
	Definir length_StringSumation Como Numero;
	Definir IsNumber Como Logico;
	num_carry 	     = "";
	i				 = 0;
	length_StringSumation  = string_Length(string_Sumation);
	index_Tcomponent	    = string_Length(dataSoup);
	new_canvas = canvas;
	si string_Length(dataSoup) < 1 Entonces
	...
	Mientras i <= length_StringSumation Hacer
		...
		Si !IsNumber & !string_isEmpty(num_carry) | Num_isEquals(i, length_StringSumation) Entonces
			num_carry_int   	 = string_ToNum(num_carry);
			si num_carry_int > 0 Entonces
				index_Tcomponent_Dec = decrement_step(index_Tcomponent, num_carry_int);
				data_Current 	     = string_substring(dataSoup, index_Tcomponent_Dec, index_Tcomponent);
				new_canvas  		 = canvas_DrawTComponent(new_canvas, Cx, Cy, data_Current);
				index_Tcomponent     = index_Tcomponent_Dec;
			SiNo
				error_Message_Funtion("__private_Tocanvas_ByDataSoup()","size tcomponent length < 1");
			FinSi
			num_carry      		 = "";// clear
		FinSi
		i = increment(i);
...
Funcion tcomponent_Build <- Tcomponent_New(type, x0, y0)
	...
	tcomponent_Build = collectionSetter_addValueFuntion(tcomponent_Build, TYPE_STRING());
...
Funcion NewTButton <- TComponent_NewButton(text, input_string, x0, y0)
	...
	NewTButton = Tcomponent_AddControlInput(NewTButton, input_string);
	NewTButton = Tcomponent_AddControlText(NewTButton, text);
FinFuncion
...
Funcion NewTComponent <- TComponent_NewLabel(text, x0, y0)
	...
	NewTComponent = Tcomponent_AddControlText(NewTComponent, text);
FinFuncion
...
Funcion NewTComponent <- TComponent_NewHorizontalRuler(x0, y0, width)
	...
	NewTComponent = Tcomponent_AddControlValue(NewTComponent, num_ToString(width));
FinFuncion
...
Funcion NewTComponent <- TComponent_NewVerticalRuler(x0, y0, height)
	...
	NewTComponent = Tcomponent_AddControlValue(NewTComponent, num_ToString(height));
FinFuncion

Funcion NewTComponent <- TComponent_NewPanel(x0, y0, x1, y1)
	Definir NewTComponent, pos_panel, pos_panel_donw, pos_final Como Texto;
	NewTComponent = Tcomponent_New(Tcomponent_TYPE_PANEL(), x0, y0);
	pos_panel = Tcomponent_GetTproperty_Size(NewTComponent);
	pos_panel_donw = TUI_ParserPointXY_ToString(x1, y1);
	pos_final = string_append_withSeparator(pos_panel, pos_panel_donw, symbol_Separator());
...
Funcion NewTComponent <- TComponent_NewSpriteString(string_Sprite, sprite_effect, x0, y0)
	...
	SiNo
		...
		error_Message_Funtion("TComponent_NewSpriteString(sprite, separator, x, y)", "sprite not is TYPE_SPRITESTRING() [use SpriteString_New()]");
	FinSi
FinFuncion
...
Funcion NewTComponent <- __private_TComponent_ForcerdNewSpriteString(string_Sprite, sprite_effect, x0, y0)
	...
	NewTComponent = Tcomponent_AddControlText(NewTComponent, string_Sprite);
	NewTComponent = Tcomponent_AddControlValue(NewTComponent, sprite_effect);
FinFuncion

// -- -- -- -- -- [ ADD ] -- -- -- -- -- --
Funcion tcomponent_Build <- Tcomponent_AddControlInput(tcomponent, input)
...
Funcion tcomponent_Build <- Tcomponent_AddControlText(tcomponent, text)
	Definir tcomponent_Build Como Texto;
    tcomponent_Build = collectionSetter_AddKeyValue_ByStringForced(tcomponent, TProperty_Text(), text);
FinFuncion

Funcion tcomponent_Build <- Tcomponent_AddControlValue(tcomponent, value)
	Definir tcomponent_Build Como Texto;
...
Funcion isTUI <- Tcomponent_IsTcomponent(interface_string)
	Definir isTUI Como Logico;
	isTUI = string_isEquals(collection_getName_TypeCollection(interface_string), TUI_TYPE_TCOMPONENT());
FinFuncion
...
// -- -- -- -- -- [ GET ] -- -- -- -- -- --
Funcion Tproperty <- Tcomponent_GetTproperty_Size(tcomponent_element)
	Definir Tproperty Como Texto;
	Tproperty = linearCollection_getFirts_ToString(tcomponent_element);
FinFuncion
...
Funcion New_TComponent <- Tcomponent_SetTproperty_Position_TwoPoints(Tcomponent, x0, y0, x1, y1) 
	Definir New_TComponent Como texto;
	tcomponent_Points = TUI_ParserTwoPointsXY_ToString(x0, y0, x1, y1);
...
Funcion New_TComponent <- Tcomponent_SetTproperty_Position(Tcomponent, x0, y0) 
	Definir New_TComponent Como texto;
	New_TComponent = Tcomponent_SetTproperty_Position_ByString(Tcomponent, TUI_ParserPointXY_ToString(x0, y0));
FinFuncion
...
Funcion canvas_Result <- canvas_DrawTComponent_Button(canvas, Cx, Cy, Tbutton)
	...
	si Tcomponent_IsTcomponent(Tbutton) Entonces
		pos_Tbutton = Tcomponent_GetTproperty_Size(Tbutton);
...
Funcion canvas_Result <- canvas_DrawTComponent_Label(canvas, Cx, Cy, Tlabel)
	...
	si Tcomponent_IsTcomponent(Tlabel) Entonces
		pos_Tlabel = Tcomponent_GetTproperty_Size(Tlabel);
...
Funcion canvas_Result <- canvas_DrawTComponent_HorizontalRuler(canvas, Cx, Cy, THrzRuler)
	...
	si Tcomponent_IsTcomponent(THrzRuler) Entonces
		pos_Thrz_ruler = Tcomponent_GetTproperty_Size(THrzRuler);
...
Funcion canvas_Result <- canvas_DrawTComponent_VerticalRuler(canvas, Cx, Cy, TVrtRuler)
	...
	Definir index_separator, x0, y0, heigth_vrz Como Numero;
	canvas_Result = canvas;
	si Tcomponent_IsTcomponent(TVrtRuler) Entonces
		pos_Tvrz_ruler = Tcomponent_GetTproperty_Size(TVrtRuler);
		...
		heigth_vrz = string_ToNum(Tcomponent_GetTproperty_Value(TVrtRuler));
		canvas_Result = canvas_DrawVerticalLine(canvas_Result, Cx, Cy, x0, y0, heigth_vrz);
	FinSi
FinFuncion

Funcion canvas_Result <- canvas_DrawTComponent_Panel(canvas, Cx, Cy, TPanel)
	Definir canvas_Result, pos_Tpanel Como Texto;
	Definir index_separator, index_separator_donw, index_separatorPos Como Numero;
	Definir x0, y0, width, heigth Como Numero;
	canvas_Result = canvas;
	si Tcomponent_IsTcomponent(TPanel) Entonces
		pos_Tpanel = Tcomponent_GetTproperty_Size(TPanel);
		index_separator = string_indexOf(pos_Tpanel, symbol_Separator_Simple());
		index_separator_donw = string_LastIndexOf(pos_Tpanel, symbol_Separator_Simple());
		index_separatorPos = string_LastIndexOf_fromIndex(pos_Tpanel, symbol_Separator(), index_separator_donw);
		...
		width = string_ToNum(string_substring(pos_Tpanel, increment(index_separatorPos), index_separator_donw));
		heigth = string_ToNum(string_substring_from(pos_Tpanel, increment(index_separator_donw)));
		canvas_Result = canvas_DrawPanel(canvas, Cx, Cy, x0, y0, width, heigth);
	FinSi
FinFuncion
...
Funcion canvas_Result <- canvas_DrawTComponent_SpriteString(canvas, Cx, Cy, TSprite)
	...
	si Tcomponent_IsTcomponent(TSprite) Entonces
		pos_TSprite = Tcomponent_GetTproperty_Size(TSprite);
...
FinFuncion
//.....
Funcion canvas_Result <- canvas_DrawArray_TComponent(canvas, Cx, Cy, array_Tcomponent, size_Array)
	Definir canvas_Result Como Texto;
	Definir i Como Numero;
	i = 0 ;
	canvas_Result = canvas;
	Mientras  i < size_Array Hacer
		canvas_Result = canvas_DrawTComponent(canvas_Result, Cx, Cy, array_Tcomponent[i]);
		i = increment(i);
	FinMientras
FinFuncion
...
Funcion vec_str <- VEC_vec_str(result_string)
	Definir vec_str Como Texto;
	vec_str = VEC_str_withSeparator(result_string, symbol_Separator());
FinFuncion

Funcion vec_str <- VEC_str_withSeparator(result_string, separator)
	Definir vec_str Como Texto;
	Definir count_vec Como Numero;
	count_vec = string_countMatches(result_string, separator);
