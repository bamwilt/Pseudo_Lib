...
Funcion result <- String_join_array_withSeparator(array,size,separator)
	...
	Para i=1 Hasta decrement(size) Hacer
		result = String_append_withSeparator(result,array[i],separator);
	FinPara
FinFuncion

Funcion result <- String_join_array_(array,size)
	Definir result Como Texto;
...
Funcion str <- __private_String_insert_general(text,text_insert,index,isReplace)
	...
	fragments[2]=after;
	str=String_join_array_(fragments,3);
FinFuncion
...
Funcion HideText
	print(String_join(Escape_Symbol(),"2J"));
FinFuncion
...
Funcion log_x <- Math_log(x, base)
    Definir log_x Como Real;
    log_x = Math_ln(x) / Math_ln(base);
FinFuncion
...
Funcion ln_x <- Math_ln(x)
	Definir ln_x, t, term Como Real;
    Definir i, n Como Entero;
	//ln_x =  Math_serie_taylor(decrement(x),decrement(x), 10,1, 1, 1, false(), true());
	t = decrement(x);
    ln_x = t;
    term = t;
    n = 1;
...
Funcion num_trunc <- Math_truncate(num)
	...
	num_trunc = if_else(String_isNumber_float(str_num),String_ToNum(String_substring_from_start(str_num,index_point)),num,TYPE_INT());
FinFuncion 
...
Funcion DrawArea_move <- TUI_DrawArea_MoveContent_XY(DrawArea, DWx, DWy, isleft, istop,speed_move)
	Definir DrawArea_move como Texto;
	DrawArea_move = TUI_DrawArea_MoveContent_X(DrawArea, DWx, DWy,isLeft,speed_move);
	DrawArea_move = TUI_DrawArea_MoveContent_Y(DrawArea_move, DWx, DWy,istop,speed_move);
FinFuncion

Funcion DrawArea_move <- TUI_DrawArea_MoveContent_Y(DrawArea, DWx, DWy, istop,speed_moven)
	...
	y_insert_position = if_else(istop, 0, DWx*DWy,TYPE_INT());//0 | 15*4 = 60
	y_remove_position= if_else(istop,decrement_step(DWx*DWy,y_insert_length), 0,TYPE_INT());//15*4=60 - 30 = 30 | 0
...
Funcion DrawArea_button <- TUI_DrawArea_DrawButton(text,DrawArea,DWx,DWy,_x,_y)
	...
	xt = increment_step(_x,length_text);
	yt= increment_step(_y,2);
	DrawArea_button = TUI_DrawArea_DrawRectangle(DrawArea, DWx, DWy, _x, _y, increment(xt), yt);
...
Funcion index_tui <- TUI_DrawArea_getIndex(x_,y_,DWx)
...
Funcion DrawArea_line <- TUI_DrawArea_DrawLine(DrawArea,DWx,DWy,x0,y0,x1,y1)
    ...
    xi = if_else(Dx >= 0, 1, -1, TYPE_INT());
    yi = if_else(Dy >= 0, 1, -1, TYPE_INT());
...
Funcion row_x <- TUI_Row_WithText(text,repeats)
	...
	Para i=1 Hasta repeats Con Paso 1 Hacer
		row_x = String_append(row_x,text);
	FinPara
FinFuncion

Funcion DrawArea <- TUI_DrawArea(x_,y_)
	Definir i Como Numero;
	Definir DrawArea, DrawArea_x Como Texto;
	DrawArea_x=TUI_Row_WithText(pixel_clear(),x_);
	Para i=1 Hasta y_ Con Paso 1 Hacer
...
Funcion vec2_str <- VEC_vec2(_x,_y)
	Definir vec2_str, XY como Texto;
	Dimension XY[2];
	XY[0] = Num_ToString(_x);
	XY[1] = Num_ToString(_y);
	vec2_str = VEC_vec_str(array_ofString_separator(XY,2,separator_Symbol()));
FinFuncion

Funcion vec3_str <- VEC_vec3(_x,_y, _z)
	Definir vec3_str, XYZ como Texto;
	Dimension XYZ[3];
	XYZ[0] = Num_ToString(_x);
	XYZ[1] = Num_ToString(_y);
